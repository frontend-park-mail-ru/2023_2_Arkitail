class Header {
    constructor(parent, data) {
        ...
        this.template = Handlebars.compile(`
            ...
            {{else}}
            <button gateway="{{urls.signup}}" class="right-menu btn-register hidden-on-mobile">
                <p>Регистрация</p>
            </button>
            <button gateway="{{urls.login}}" class="right-menu btn-login">
    ...
    render() {
        this.parent.innerHTML = this.template({
            ...
            urls: this.data.urls, authorized: context.authenticated.status
        });
    ...
    authRender() {
        authenticate().then(() => header.render());
    }
}
